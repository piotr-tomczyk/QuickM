...
def main():
    ...
    root.greet_button = customtkinter.CTkButton(
        text="Odpal Server", command=lambda: serverInit(root), state=NORMAL
    )
    root.greet_button.pack()
    root.my_label = customtkinter.CTkLabel(root, textvariable=my_string_var)
    root.my_label.pack()
    root.greet_button2 = customtkinter.CTkButton(
        text="Wyslij wiadomość", command=serverInit
    )
